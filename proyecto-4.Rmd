...
for (i in 1:length(team_stats$Athlete)){
...
p1 <- team_stats %>%
  ...
  filter(Athlete==team_stats$Athlete[i], Statistic=="dis_total")%>%
...
p2 <- team_stats %>%
  select(Athlete, dis_total, sd, validos, diferencia)%>%
  gather(key="Statistic", value="Aggregate", -Athlete)%>%
  filter(Athlete==team_stats$Athlete[i], Statistic=="sd")%>%
  ggplot(.,aes(x=Statistic, weights=Aggregate))+
  geom_bar()+
...
p3 <- team_stats %>%
  select(Athlete, dis_total, sd, validos, diferencia)%>%
  gather(key="Statistic", value="Aggregate", -Athlete)%>%
  filter(Athlete==team_stats$Athlete[i], Statistic=="validos")%>%
  ggplot(.,aes(x=Statistic, weights=Aggregate))+
  geom_bar()+
...
p4 <- team_stats %>%
  select(Athlete, dis_total, sd, validos, diferencia)%>%
  gather(key="Statistic", value="Aggregate", -Athlete)%>%
  filter(Athlete==team_stats$Athlete[i], Statistic=="diferencia")%>%
  ggplot(.,aes(x=Statistic, weights=Aggregate))+
  geom_bar()+
...
grid.arrange(arrangeGrob(grobs= list(p1,p2,p3,p4), layout_matrix = lay, vp = NULL, top = team_stats$Athlete[i]))
}
